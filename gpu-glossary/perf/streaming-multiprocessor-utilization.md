...
[`nvidia-smi`](/gpu-glossary/host-software/nvidia-smi.md), but more fine-grained.
Instead of reporting the fraction of time that a
[kernel](/gpu-glossary/device-software/kernel.md) is executing anywhere on the GPU,
it reports the fraction of time all
[SMs](/gpu-glossary/device-hardware/streaming-multiprocessor.md) spend executing
[kernels](/gpu-glossary/device-software/kernel.md). If a
[kernel](/gpu-glossary/device-software/kernel.md) uses only one
[SM](/gpu-glossary/device-hardware/streaming-multiprocessor.md), e.g. because it
only has one [thread block](/gpu-glossary/device-software/thread-block.md), then it
will achieve 100% GPU utilization while it is active, but the SM utilization
will be at most one over the number of
[SMs](/gpu-glossary/device-hardware/streaming-multiprocessor.md) — under 1% in an
H100 GPU.
...
[pipe utilization](/gpu-glossary/perf/pipe-utilization.md), which measures how
effectively each SM uses its internal functional units. High SM utilization with
low [pipe utilization](/gpu-glossary/perf/pipe-utilization.md) indicates that your
[kernel](/gpu-glossary/device-software/kernel.md) is running on many SMs but not

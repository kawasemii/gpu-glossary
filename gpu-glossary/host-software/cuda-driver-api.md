...
allocating [memory](/gpu-glossary/device-software/global-memory.md) on GPU devices,
for example.

![The CUDA Toolkit. The CUDA Driver API sits between applications or other toolkit components and the GPU. Adapted from the *Professional CUDA C Programming Guide*.](../resources/terminal-cuda-toolkit.svg)

Very few CUDA programs are written to directly use the CUDA Driver API. They
instead use the
[CUDA Runtime API](/gpu-glossary/host-software/cuda-runtime-api.md). See
...
[libcuda.so](/gpu-glossary/host-software/libcuda.md) on Linux systems.
...
For details on distributing [CUDA C/C++](/gpu-glossary/host-software/cuda-c.md)

...
![In high-performance data center GPUs like the H100, RAM is located on a die directly next to the processor's. Adapted from the Wikipedia page for [high-bandwidth memory](https://en.wikipedia.org/wiki/High_Bandwidth_Memory).](../resources/terminal-hbm-schematic.svg)
...
(SRAM) used in [registers](/gpu-glossary/device-hardware/register-file.md) and
[cache memory](/gpu-glossary/device-hardware/l1-data-cache.md). For details on DRAM
and SRAM, we recommend Ulrich Drepper's 2007 article
["What Every Programmer Should Know About Memory"](https://people.freebsd.org/~lstewart/articles/cpumemory.pdf) ([local copy here](../resources/cpumemory.pdf)).

It is generally not on the same die as the
[SMs](/gpu-glossary/device-hardware/streaming-multiprocessor.md), though in the
...
increased [bandwidth](/gpu-glossary/perf/memory-bandwidth.md). These GPUs use
...
[global memory](/gpu-glossary/device-software/global-memory.md) of the
[CUDA programming model](/gpu-glossary/device-software/cuda-programming-model.md)
and to store [register](/gpu-glossary/device-software/registers.md) data that
spills from the [register file](/gpu-glossary/device-hardware/register-file.md).

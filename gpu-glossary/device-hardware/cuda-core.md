...
The CUDA Cores are GPU [cores](/gpu-glossary/device-hardware/core.md) that execute
scalar arithmetic instructions.

![The internal architecture of an H100 SM. The CUDA Cores and Tensor Cores are depicted in green. Note the larger size and lower number of Tensor Cores. Modified from NVIDIA's [H100 white paper](../resources/gtc22-whitepaper-hopper.pdf).](../resources/terminal-gh100-sm.svg)

They are to be contrasted with the
[Tensor Cores](/gpu-glossary/device-hardware/tensor-core.md), which execute matrix
operations.
...
[Warp Scheduler](/gpu-glossary/device-hardware/warp-scheduler.md) but apply that
instruction to different [registers](/gpu-glossary/device-software/registers.md).
Commonly, these groups are of size 32, the size of a
[warp](/gpu-glossary/device-software/warp.md), but for contemporary GPUs groups can
...
[CUDA Device Architecture](/gpu-glossary/device-hardware/cuda-device-architecture.md)).

So, for example, the
[H100 whitepaper](https://resources.nvidia.com/en-us-hopper-architecture/nvidia-h100-tensor-c) ([local copy here](../resources/gtc22-whitepaper-hopper.pdf))
